...
class Compute_measures(object):
    ...
    def gather_all_intervener_types(trees):
        all_intervener_types = []

        for tree in trees:
            measures = Compute_measures(tree)
            for edge in tree.edges:
                intervener_types = measures.intervener_pos(edge)
                all_intervener_types.extend(intervener_types)

        return all_intervener_types

    def plot_histogram(intervener_types):
        counter = Counter(intervener_types)
        labels, values = zip(*counter.items())
        plt.bar(labels, values)
        plt.xlabel('Parts of Speech')
        plt.ylabel('Frequency')
        plt.title('Distribution of Intervener Types')
        plt.show()
    ...
    def head_pos(self, edge):
            print(f"Processing edge: {edge}")
            kind = []
            pos_head = self.tree.nodes[edge[0]].get('upostag')
            print(f"Found pos_head: {pos_head} for head node {edge[0]}")
            if pos_head is not None:
                kind.append(pos_head)
            return kind
        ...
        for nodex in nodex_between:
            for in_edge in self.tree.in_edges(nodex):
                ...
                if self.tree.has_edge(head_node, nodex):
                    int_rel = self.tree.nodes[nodex].get('deprel')
                    if pos_nodex is not None:
                        types.append(pos_nodex)
                        rel.append(int_rel)
